        ...
        for dataset in datasetList:
            for fc in arcpy.ListFeatureClasses("", "", dataset):
                try:
                    ...
                    if (
                        arcpy.Describe(dataset).dataType == "Coverage"
                        or arcpy.Describe(dataset).dataType == "CadDrawingDataset"
                        or arcpy.Describe(dataset).dataType == "Workspace"
                    ):
                    ...
                    if (
                        ...
                        fcFeatureCount = str(
                            arcpy.GetCount_management(dataset + "/" + fc)
                        )
                    else:
                    ...
                    print(
                        ...
                        + fc
                        + " at: "
                        + os.path.join(subFolderName, dataset)
                        + "; Skipping"
                    )
                    arcpy.AddMessage(
                        "     Could not process: "
            ...
            try:
                if arcpy.Describe(fc).dataType != "CoverageFeatureClass":
                    ...
                    if tree.find("Esri/ArcGISFormat") is not None:
                        ...
                        arcpy.AddMessage(
                            "     Processing File: " + fc + " (Version 10.x XML)"
                        )
                        title = tree.find("dataIdInfo/idCitation/resTitle")
                        keywords = tree.find("dataIdInfo/searchKeys/keyword")
                        abstract = tree.find("dataIdInfo/idAbs")
                        ...
                        arcpy.AddMessage(
                            "     Processing File: " + fc + " (Version 9.x XML)"
                        )
                        title = tree.find("idinfo/citation/citeinfo/title")
                        keywords = tree.find("idinfo/keywords/theme/themekey")
                        abstract = tree.find("idinfo/descript/abstract")
        ...
        for table in tableList:
            try:
                ...
                if tree.find("Esri/ArcGISFormat") is not None:
                    ...
                    arcpy.AddMessage(
                        "     Processing File: " + table + " (Version 10.x XML)"
                    )
                    title = tree.find("dataIdInfo/idCitation/resTitle")
                    keywords = tree.find("dataIdInfo/searchKeys/keyword")
                    abstract = tree.find("dataIdInfo/idAbs")
                    ...
                    arcpy.AddMessage(
                        "     Processing File: " + table + " (Version 9.x XML)"
                    )
                    title = tree.find("idinfo/citation/citeinfo/title")
                    keywords = tree.find("idinfo/keywords/theme/themekey")
                    abstract = tree.find("idinfo/descript/abstract")
                ...
                for fc in arcpy.ListFeatureClasses("", "", item):
                    try:
                        ...
                        print(
                            ...
                            + fc
                            + " at: "
                            + os.path.join(subFolderName, workspace, item)
                            + "; Skipping"
                        )
                        arcpy.AddMessage(
                            "     Could not process: "
                        ...
                        try:
                            ...
                            print("     Could not remove file: " + str(fcXML))
                            arcpy.AddMessage(
                                "     Could not remove file: " + str(fcXML)
                            )
            ...
            for fc in arcpy.ListFeatureClasses():
                try:
                    ...
                    print(
                        ...
                        + fc
                        + " at: "
                        + os.path.join(subFolderName, workspace)
                        + "; Skipping"
                    )
                    arcpy.AddMessage(
                        "     Could not process: "
            ...
            for raster in rasterset:
                try:
                    ...
                    if tree.find("Esri/ArcGISFormat") is not None:
                        ...
                        arcpy.AddMessage(
                            "     Processing File: " + raster + " (Version 10.x XML)"
                        )
                        title = tree.find("dataIdInfo/idCitation/resTitle")
                        keywords = tree.find("dataIdInfo/searchKeys/keyword")
                        abstract = tree.find("dataIdInfo/idAbs")
                        ...
                        arcpy.AddMessage(
                            "     Processing File: " + raster + " (Version 9.x XML)"
                        )
                        title = tree.find("idinfo/citation/citeinfo/title")
                        keywords = tree.find("idinfo/keywords/theme/themekey")
                        abstract = tree.find("idinfo/descript/abstract")
                    ...
                    try:
                        ...
                        arcpy.AddMessage(
                            "      Could not remove file: " + str(rasterXML)
                        )
